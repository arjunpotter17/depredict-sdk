...
export function CreateMarketCreatorFlow({ onCreated }: CreateMarketCreatorFlowProps) {
  ...
  const { verifyMarketCreator, client, ensureMarketCreatorLookupTable } = useShortx()
  const { createCollection } = useCreateCoreCollection()
  const { createTree } = useCreateMerkleTree()
  const [step, setStep] = useState<
    'ready' | 'creating' | 'creating-collection' | 'creating-tree' | 'verifying' | 'creating-lookup-table' | 'complete'
  >('ready')
  ...
  const handleCreateAndVerify = async () => {
    ...
    try {
      ...
      // Wait for verification to settle
      await new Promise((resolve) => setTimeout(resolve, 2000))

      // Step 5: Create Market Creator Lookup Table
      setStep('creating-lookup-table')
      toast.loading('Step 5: Creating market creator lookup table...', { id: 'mc-flow' })

      const lookupTableResult = await ensureMarketCreatorLookupTable({
        authority: wallet.publicKey,
        payer: wallet.publicKey,
      })

      if (!lookupTableResult) {
        throw new Error('Failed to create market creator lookup table')
      }

      console.log('✅ Lookup table address:', lookupTableResult.lookupTableAddress.toBase58())

      // Handle create transaction if it exists
      if (lookupTableResult.createTx) {
        const createBlockhash = await connection.getLatestBlockhash()
        lookupTableResult.createTx.message.recentBlockhash = createBlockhash.blockhash

        const signedCreateTx = await wallet.signTransaction(lookupTableResult.createTx)
      ...
      // Handle extend transactions
      if (lookupTableResult.extendTxs && lookupTableResult.extendTxs.length > 0) {
        for (let i = 0; i < lookupTableResult.extendTxs.length; i++) {
          const extendTx = lookupTableResult.extendTxs[i]
          const extendBlockhash = await connection.getLatestBlockhash()
          extendTx.message.recentBlockhash = extendBlockhash.blockhash

          toast.loading(`Extending lookup table (${i + 1}/${lookupTableResult.extendTxs.length})...`, { id: 'mc-flow' })
          ...
          console.log(`✅ Lookup table extended ${i + 1}/${lookupTableResult.extendTxs.length}:`, extendSignature)

          // Wait between extends
          if (i < lookupTableResult.extendTxs.length - 1) {
            await new Promise((resolve) => setTimeout(resolve, 2000))
          }
        }
      }

      console.log('✅ Market creator lookup table setup complete')
      ...
      const marketCreatorDetails = {
        ...
        lookupTableAddress: lookupTableResult.lookupTableAddress.toBase58(),
        verified: true,
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-slate-400 text-sm mb-4">
              To create and manage prediction markets, you need to set up a Market Creator account. This is a one-time
              process that will:
            </p>
      ...
      {step === 'ready' && (
        <div className="p-6 rounded-2xl bg-slate-800/50 border border-slate-700/50">
          ...
          <p className="text-slate-400 mb-6">
            Click the button below to start the setup process. This will use your connected wallet as the admin and take
            a few moments with multiple transactions.
          </p>
          ...
          {wallet.publicKey && (
            <div className="p-4 rounded-xl bg-slate-900/50 border border-slate-700/30 mb-6">
              ...
              <p className="font-mono text-sm text-purple-400 break-all">{wallet.publicKey.toBase58()}</p>
            </div>
          )}
          ...
          >
            {!wallet.publicKey ? 'Connect Wallet to Continue' : 'Create & Verify Market Creator'}
          </button>
        </div>
      )}
      ...
      {(step === 'creating' ||
        ...
        step === 'creating-tree' ||
        step === 'creating-lookup-table') && (
        <div className="p-6 rounded-2xl bg-slate-800/50 border border-slate-700/50">
          <div className="flex items-center gap-3 mb-6">
            <Loader2 className="w-6 h-6 text-purple-500 animate-spin" />
            <h2 className="text-2xl font-bold">
              {step === 'creating'
                ? 'Creating Market Creator...'
                : step === 'creating-collection'
                  ? 'Creating Core Collection...'
                  : step === 'creating-tree'
                    ? 'Creating Merkle Tree...'
                    : step === 'verifying'
                      ? 'Verifying Market Creator...'
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-3 p-3 rounded-lg bg-slate-900/50">
              ...
              <span className="text-sm">Create Lookup Table</span>
            </div>
          </div>
        ...
        <div className="p-6 rounded-2xl bg-green-500/10 border border-green-500/20">
          ...
          <div className="mb-6 p-5 rounded-xl bg-slate-900/50 border border-slate-700/50">
            ...
            <p className="text-sm text-slate-400 mb-4">
              Copy the following lines and add them to your{' '}
              <code className="px-1.5 py-0.5 bg-slate-800 rounded text-purple-400">.env.local</code> file:
            </p>
            ...
            <div className="space-y-3">
              ...
              <div className="relative group">
                ...
                <pre className="p-4 rounded-lg bg-slate-950 border border-slate-700 overflow-x-auto text-xs">
                  <code className="text-green-400">NEXT_PUBLIC_CREATOR_PUBLIC_ADMIN_KEY={marketCreatorPda}</code>
                </pre>
                <button
                  onClick={() =>
                    copyToClipboard(`NEXT_PUBLIC_CREATOR_PUBLIC_ADMIN_KEY=${marketCreatorPda}`, 'Market Creator env')
                  }
              ...
              <div className="relative group">
                ...
                <pre className="p-4 rounded-lg bg-slate-950 border border-slate-700 overflow-x-auto text-xs">
                  <code className="text-green-400">NEXT_PUBLIC_SHORTX_COLLECTION_ADDRESS={coreCollection}</code>
                </pre>
                <button
                  onClick={() =>
                    copyToClipboard(`NEXT_PUBLIC_SHORTX_COLLECTION_ADDRESS=${coreCollection}`, 'Collection env')
                  }
                  className="absolute top-6 right-2 p-2 rounded-lg bg-slate-800 hover:bg-slate-700 border border-slate-600 transition-all opacity-0 group-hover:opacity-100"
                >
          ...
          <div className="mb-6 p-5 rounded-xl bg-slate-900/50 border border-slate-700/50">
            ...
            <div className="p-4 rounded-lg bg-slate-950 border border-slate-700">
              <code className="text-sm text-slate-300">npm run dev</code>
            </div>
          </div>
